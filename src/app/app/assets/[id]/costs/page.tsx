  ...
  return (
    <>
      ...
      <main className="flex flex-1 flex-col gap-5 px-4 py-6">
        ...
        <section>
          <h2 className="mb-2 text-sm font-medium text-neutral-700">비용 목록</h2>
          <CostList entries={asset.costs} members={asset.members} onDelete={(costId) => deleteCost(asset.id, costId)} />
        </section>

        <section className="rounded-md border border-neutral-200 p-4">
          <h2 className="mb-2 text-sm font-medium text-neutral-700">참여자별 낸 비용 합계</h2>
          <ul className="flex flex-col gap-1 text-sm">
            ...
            <li className="mt-1 flex justify-between border-t border-neutral-200 pt-1 font-medium">
              <span>전체 합계</span>
              <span>{won(totalCost)}</span>
            </li>
          </ul>
          <p className="mt-2 text-xs text-neutral-500">정산에는 반영되지 않습니다.</p>
        </section>
      </main>
    </>
  );
